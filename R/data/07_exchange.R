...
# to the first of the month. All §F series are Δln in the model, so log-levels
# are stored here (differencing happens in the model assembly), matching prices.
#
...
# is an index. GBP is pulled alongside as a bonus major.
fx_tbl <-
  cbi_monthly(c(
  ...
  mutate(across(c(isk_eur, isk_usd, isk_gbp, twi), log))
...
# appreciation (less competitive). Monthly from 1994. Stored as a log-level.
reer_tbl <-
  read_csv(
